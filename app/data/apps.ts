...
export const apps: App[] = [
  ...
  },
  {
    ...
    longDescription: `복잡한 명령어 없이 버튼 클릭 몇 번으로 만드는, 가장 자연스러운 한국인 옷핏 레퍼런스.\n\nJoA는 Google Gemini 이미지 생성 모델을 활용합니다. 카테고리(남/여/풍경), 연령, 체형, 스타일, 날씨 무드를 선택하면 9:16 세로 이미지가 바로 생성됩니다. 현재 날씨를 반영한 이미지 생성도 가능하며, 생성된 이미지는 히스토리로 저장됩니다.\n\n🌐 웹앱으로도 이용 가능\n앱 설치 없이 Android, iPhone, PC 브라우저 어디서든 동일하게 사용할 수 있습니다.\n\n🎁 무료로 먼저 써보세요\n가입이나 로그인 없이 즉시 5장을 무료로 테스트할 수 있습니다.\n\n🗓️ 1개월 이용권 혜택\n하루 30장 생성 권한이 매일 자정에 자동 충전됩니다. 30일 기준 최대 900장까지 사용할 수 있습니다. 미사용 분은 다음 날로 이월되지 않습니다. 인증 코드는 스마트폰, PC 등 여러 기기에서 동시에 사용할 수 있지만, 하루 30장 카운트는 기기 구분 없이 공유됩니다.\n\n🔑 개인 API Key 직접 연동 (BYOK)\nGoogle AI 스튜디오(aistudio.google.com)에서 Gemini API Key를 직접 발급받아 앱 우측 상단 ⚙️ 설정에 입력하면 이용할 수 있습니다. 이 경우 앱 개발자에게 별도 비용을 지불하지 않지만, Google API 사용량에 따라 구글에 직접 요금이 부과될 수 있습니다. API Key는 길어서 입력하기 불편하므로, 이 사이트의 QR 변환 메뉴를 이용해 QR 코드로 변환한 뒤 앱에서 스캔하면 편리합니다.\n\n📲 설치 방법 (Android)\n위 'Android 직접 설치' 버튼으로 APK 파일을 다운로드한 후, 파일을 열어 설치합니다. 처음 설치 시 '알 수 없는 앱 설치 허용' 설정이 필요할 수 있습니다.\n\n🗑️ 삭제 방법\n일반 앱과 동일하게 앱 아이콘을 길게 누른 후 '제거'를 선택하거나, 설정 > 앱에서 삭제할 수 있습니다.`,
    api: ['Gemini'],
